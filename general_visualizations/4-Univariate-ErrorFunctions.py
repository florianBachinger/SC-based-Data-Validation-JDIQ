...
normal = sef.Normal(num, start=num*0.2, end=num*0.8) 
...
ax[0].text(50,1.03, "$\psi=0.6$", ha='center')

ax[0].vlines(x=xphi, ymin=0, ymax=1,  color=pal2[7], ls=':', lw=1.5, label='scaling $\phi$')
ax[0].text(xphi+1,0.5, "$\phi=1$")

# ----------  Square - width ----------
ax[1].plot(x,square, c=pal2[7])

ax[1].plot(np.linspace(num*0.4, num*0.6, num),[1.2]*num,ls='--', lw=1.5, c=pal2[7], label='width $\psi$')
ax[1].text(50,1.03, "$\psi=0.2$", ha='center')

ax[1].vlines(x=xphi, ymin=0, ymax=1,  color=pal2[7], ls=':', lw=1.5, label='scaling $\phi$')
ax[1].text(xphi+1,0.5, "$\phi=1$")
...
ax[2].plot(np.linspace(num*0.2, num*0.8, num),[1.2]*num,ls='--', lw=1.5, c=pal1[7])
ax[2].text(50,1.03, "$\psi=0.6$", ha='center')

ax[2].vlines(x=xphi, ymin=0, ymax=1,  color=pal3[7], ls=':', lw=1.5)
ax[2].text(xphi+1,0.5, "$\phi=1$")
...
plt.savefig('figures/experimental_setup/univariate_error_functions.pdf', dpi = 600)
plt.show()
