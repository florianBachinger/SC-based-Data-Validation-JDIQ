...
curves_results = pd.read_csv('3.1.4-rocCurveResults.csv')
plt.figure(figsize=(10,8))

curves_results = curves_results[curves_results['DataSize'] == 50]
  ...
  for error_width_percentage in np.unique(curves_results['ErrorWidthPercentage']):
    for noise_level_percentage in np.unique(curves_results['NoiseLevelPercentage']):
      for error_scaling_sigma in np.unique(curves_results['ErrorScaling']):
        ...
        plt.savefig(f'fig/3.1.4-roc_size{data_size}_errWid{error_width_percentage}_noiseLev{noise_level_percentage}_errScal{error_scaling_sigma}.png', transparent = True, dpi = 600)
