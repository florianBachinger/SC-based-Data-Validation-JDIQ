...
result_figure = 'figures/multivariate/multivariate-1-gridsearch_results_paper.png'
...
f, ax = plt.subplots(len(max_interactions_values),len(alpha_values),  sharex=True, sharey=True, figsize=(5.5,1.8))
plt.subplots_adjust(left=0.15, bottom=0.37, right=0.85, top=0.99, wspace=0.05, hspace=0.1)
...
for Alpha in np.unique( df['Alpha']):
  ...
  hm = sns.heatmap(data=pivot, ax=ax[col], norm=norm, cmap=cmap, cbar_ax=cbar_ax)
